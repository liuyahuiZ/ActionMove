...
class Video extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      hasTransForm: this.props.hasTransForm,
      isFullScreen: false,
    };
  ...
  changeCurrentTime(percent){
    console.log('onPercentChange', percent);
    const {  duration } = this.state;
    if (duration && percent) {
      const allTime = this.fixedStr(Number(duration));
      let newCurrent = this.fixedStr(Number((percent * allTime) / 100));
      this.playBySeconds(newCurrent);
    }
  };
  toggle(){
    const { isFullScreen } = this.state;
    const status = !isFullScreen;
    this.props.fullScreenClick && this.props.fullScreenClick(status);
    this.setState({
      isFullScreen: status,
    })
  }
  render() {
    const { playStatus, current, duration, preUrl, souceUrl, hasTransForm } = this.state;
    ...
    return (
      <span  className={`video-container relative ${className}`}>
        <div className="absolute control">
          ...
          {
            playStatus === 'PLAY' && <span onClick={()=>{this.pause()}} className="opacity-0"><Icon iconName='ios-pause' size="300%" /></span>
          }
          {
            playStatus === 'PAUSE' && <span onClick={()=>{this.play()}}><Icon iconName='ios-play' size="300%" /></span>
          }
          {
            playStatus === 'END' && <span onClick={()=>{this.rePlay()}}><Icon iconName="ios-refresh-outline" size="300%" /></span>
          }
        </div>
        {this.props.children}
