    ...
    render() {
        ...
        let articleDom = articlesArr&&articlesArr.length>0 ? articlesArr.map((itm, idx)=>{
          ...
          ><Col className="textclolor-333 margin-bottom-3r" span={24}>
                <Row justify={'center'}>
                    {isPhone && <Col span={24} className='overflow-hide maxheight-10 border-radius-9 margin-bottom-1r'><ImageBird imgName={itm.imgGroup} className="img-hover border-radius-9"  /></Col>}
                    <Col span={isPhone ? 24 : 18}><Row className={!isPhone && "padding-right-2r"}>
                    ...
                    {!isPhone && <Col span={6} className='overflow-hide maxheight-10 border-radius-9'><ImageBird imgName={itm.imgGroup} className="img-hover border-radius-9"  /></Col>}
